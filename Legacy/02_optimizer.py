...
for f, configs in enumerate(configf):
    ...
    stream = subprocess.run([gcsimpath+"\gcsim.exe", '-c=' + configpath+"\\"+configf[f],"--substatOptim=true","-out="+ configpath+"\_Optimized_"+configf[f]], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
